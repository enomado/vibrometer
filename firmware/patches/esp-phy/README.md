...
> Vendored copy of `esp-phy` from [esp-rs/esp-hal](https://github.com/esp-rs/esp-hal),
> patched to fix a PHY clock ref-count leak in `disable_phy()` and wired in through
> `[patch]` in `firmware/Cargo.toml`. Upstream license below applies unchanged.
